<div id="tabs-Curso">
    <?if($cantCursos == 1):?>
        <input type="hidden" id="curso" value="<?=$idCurso;?>"></input>
        <?for($j=0;$j<$cantCursos;$j++):?>
            <?foreach(${"datosCurso".$j} as $row):?>
                <label class="msjOK">Curso <?=$row->NOMBRE.' '.$row->LETRA;?></label>
            <?endforeach;?>
        <?endfor;?>
    ...
    <?else:?>
        <div class="tipsTabs" style="float:none; width: 40%;">
            ...
            <select id="seleccionCurso" class="ui-corner-all ancho180">
                <option selected></option>
                <?for($j=0;$j<$cantCursos;$j++):?>
                    <?foreach(${"datosCurso".$j} as $row):?>
...
<script>
    ...
    $("#seleccionCurso").change(function(){
        $.post(
            base_url+'sigeca/cargaCursoEspecifico',
            {
                curso:$("#seleccionCurso").val()
            },function(htmlresponse,data)
                {
                    $("#accordionCursos2").html(htmlresponse,data);
                }
        );
    });
